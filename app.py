...
def main():
    ...
    st.title("IPL Specs")
    image = Image.open("iplimage1.jpg")

    st.image(
        image,
        caption="Lets Watch IPL together ",
    ...
    st.write("Lets plot :sunglasses:")
    st.write(
        "if you want to save this plots you can do it by clicking the circle near the plot"
    )

    if st.button("Click here for the Generate Plot"):
        st.success(
            "Produce plot for {}  and {}".format(
    ...
    if st.checkbox("check here for the Correlation Polt"):
        st.write(sns.heatmap(df.corr(), annot=True))
        st.pyplot()

    if st.checkbox("check here for the Value count"):
        st.text("Value count by Target")
        all_columns_names = df.columns.tolist()
        primary_col = st.selectbox(
            "Primary column to GroupBy", all_columns_names)
        selected_columns_names = st.multiselect(
            "Select Columns", all_columns_names)
        if st.button("Click here for the Plot"):
